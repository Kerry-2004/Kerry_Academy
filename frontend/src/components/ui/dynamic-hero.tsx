...
const parseRgbColor = (colorString?: string | null): RgbColor | null => {
...
const HeroSection = ({
  ...
  const drawArrow = useCallback(() => {
    ...
    // Distance from the cursor to the button's edge (not its far arrow-tip offset point),
    // so the fade-in starts right outside the button instead of ~500px away.
    const distanceToCenter = Math.hypot(cx - x0, cy - y0);
    const distanceToEdge = distanceToCenter - Math.max(rect.width, rect.height) / 2;
    const opacity = Math.max(0, Math.min(1, (distanceToEdge - 16) / 200));
